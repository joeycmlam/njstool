...
class ReferenceValidator:
    ...
    def load_reference_file(self, field_name: str, file_path: str, value_column: str = "code") -> bool:
        """
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                # Detect delimiter (usually comma, but handle others)
                sample = f.read(1024)
                f.seek(0)
            ...
            self.logger.info(f"Loaded {len(valid_values)} reference values for {field_name} from {file_path}")
            return True
